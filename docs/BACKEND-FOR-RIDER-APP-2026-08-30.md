...
[{ "id","ride_id","amount_pence":865,"currency":"GBP","status","provider","provider_payment_id",
   "created_at","pickup_label":"Queen Square, Wolverhampton","dropoff_label":"…" }] // labels nullable
```
> ⚠️ **Card brand/last4 (e.g. "Visa ••8901") is NOT available** — the payment
> method's details are stored nowhere. Surfacing it needs a schema + charge-path
> change (capture the PM brand/last4 from Stripe at charge time). **Decision needed**
> before it can be built; don't design the row to require it yet.
